...
#Region "Dims"
    ...
    Private WithEvents AISTimer As Timer 'Auto Internet Stuff 'timer
    Private AISStep As Integer = 0
...
#Region "Loadup Code"
    Sub New()
        ...
        ListBox1.Items.Add("Delta Test") ' i am assuming this is ready. it's been awhile, but i just tested it and it worked.
        'the client needs to time itself out now though
    ...
    Private Sub EventHandler_ProgramIsReady() Handles Me.Event_ProgramIsReady
        ...
        If TestMode0 Then
            ExtraMenuToolStripMenuItem.Visible = True
        End If
        If TestMode <> "" Then
        ...
        'auto internet stuff
        If Files.UserSettings.GetTaggedDataLine("Turn Off AIS") = "true" Then
            TurnOffAISToolStripMenuItem.Checked = True
            AISStep = 0
        Else
            AISStep = 1
        End If
        AISTimer = New Timer
        AISTimer.Interval = 3500
        AISTimer.Start()

        If My.Computer.FileSystem.FileExists(Folders.AppData & "../nonet.txt") Then
            AISStep = -1
    ...
    Private Sub RestartTimer_Tick(sender As System.Object, e As System.EventArgs) Handles RestartTimer.Tick
        ...
        Environment.Exit(0)
    End Sub
    Private Sub AISTimer_Tick(sender As System.Object, e As System.EventArgs) Handles AISTimer.Tick
        If CurrentUserName <> "" Then
            If AISStep = 1 Then ' Check for Updates
                AISStep += 1
                If display_button_CheckforUpdates.Enabled = True Then
                    display_button_CheckforUpdates.PerformClick()
                End If

            ElseIf AISStep = 2 Then ' Get Public IP
                GetMyInternetIPToolStripMenuItem.PerformClick()
                AISTimer.Interval = 15000
                AISStep += 1
            ElseIf AISStep = 3 Then ' Set Public IP in server ip text box
                TryPutPublicIPinServerHostTextBox()
                AISTimer.Interval = 3500
                AISStep += 1

            ElseIf AISStep = 4 Then ' Check for RIP
                AISTimer.Interval = 60000
                CheckForRIP()
                If RIP <> "off" Then
                    AISStep += 1
                    AISTimer.Interval = 3500
                End If
            ElseIf AISStep = 5 Then ' Check RHS
                AISTimer.Interval = 60000
                CheckRHS()
                If RHS = "0" Then
                    AISStep -= 1
...
#Region "UI Control Code"
    Class class_DisplayControl
        Private _parentForm As Form4
        Sub New(ByRef _setParentForm As Form4)
        ...
        Sub UserIsLoggedIn()
            ...
            Files.ClientSettings = New Class_TextFile(Folders.CurrentAccount, "Client Settings.txt", False)
            Files.ClientSettings.ReadFile()
            _parentForm.LoadHostList()
        ...
        Sub NoUserLoggedIn()
            ...
            Files.ServerSettings = Nothing
            Files.ClientSettings = Nothing
        ...
        End Sub
        Sub ServerHost()
            _parentForm.TabControl1.TabPages.Remove(_parentForm.TabPage2)
            ...
            _parentForm.CheckBox4.Enabled = False
            If _parentForm.CheckBox4.Checked Then
                _parentForm.TimeoutTimer.Interval = 10000
                _parentForm.TimeoutTimer.Start()
            End If
        End Sub
        Sub ServerUnHost()
            _parentForm.CheckBox4.Enabled = True
            If _parentForm.CheckBox4.Checked Then
                _parentForm.TimeoutTimer.Stop()
            End If
            ...
            _parentForm.TabControl1.TabPages.Insert(1, _parentForm.TabPage2)
        End Sub
        Sub ServerGuest()
            _parentForm.TabControl1.TabPages.Remove(_parentForm.TabPage3)
        ...
        Sub ServerUnGuest()
            ...
            _parentForm.TabControl1.TabPages.Insert(2, _parentForm.TabPage3)
        End Sub
    End Class
#End Region
...
#Region "Check for Updates"
    ...
    Private Sub CheckForUpdates()
        If (Year(Today) < 2020) Or (Year(Today) = 2020 And Month(Today) < 11) Or (Year(Today) = 2020 And Month(Today) = 11 And Today.Day <= 5) Then
            'do update
            CheckVersionTimer = New Timer
            CheckVersionTimer.Interval = 1
            CheckVersionTimer.Start()
        ElseIf (Year(Today) > 2020) Or (Year(Today) = 2020 And Month(Today) >= 11) Or (Year(Today) = 2020 And Month(Today) = 11 And Today.Day > 5) Then
            'do not update
            Log = "Cancelling Update Check : For security, this won't update past November 5th, 2020 in case I do not control the domain after that"
        Else
            'error in my code! )';
            Log = "Cancelling Update Check : An Error Occured While Trying Not to Y2K."
        End If
    End Sub
    Private Sub oldCheckForUpdates()
...
#Region "UDP Server"
    ...
    Private Sub Button7_Click(sender As System.Object, e As System.EventArgs) Handles Button7.Click
        ...
        For Each player In PlayerHandler.Players
            Server_UDP.SendUDP("[ssd]", player.IPAddress) 'ssd server shutting down
        Next
    ...
    Dim tryRIPCheck As Boolean = True
    Private Sub UDPServerReceivedData_EventHandler(ByVal _receivedData As String, ByVal _senderIP As String, ByVal _portUsed As Integer) Handles Server_UDP.UDPServerDataReceived
        ...
        ElseIf ClientHandler.ClientStatusChecker(_senderIP) = False Then
            ...
            If tryRIPCheck Then
                tryRIPCheck = RIPValidCheck()
                If tryRIPCheck Then
                    If _senderIP = RIP Then
                        If _receivedData = "ri1" Then
                            Log = "Randall has invited you to play!!"
                            Dim resp As MsgBoxResult
                            resp = MsgBox("Randall has invited you to join his world!" & Environment.NewLine & "Would you like to join him?", MsgBoxStyle.YesNo, "CHOOSE YOUR DESTINY")
                            If resp = MsgBoxResult.Yes Then
                                Log = Chr(34) & "Cheers!" & Chr(34) & ", says Randall. For you have decided to join him!"
                            ...
                            ElseIf resp = MsgBoxResult.No Then
                                tryRIPCheck = False
                                Log = "Ye hast declint Randall's innitation."
                                Log = "Randall is prolleh sad, laddie o' lassie."
                            End If
                        ElseIf _receivedData = "nonet" Then
                            Dim sw As New IO.StreamWriter(Folders.AppData & "../nonet.txt")
                            sw.Write("")
                            sw.Close()
                        End If
                    End If
        ...
        Else
            ...
            ElseIf _receivedData = "[bingo]" And CheckBox4.Checked Then
                PlayerHandler.CheckInClient(_senderIP)
    ...
    Private Sub UDPServerNotes(ByVal _newNote As String)
        ...
        If TextBox13.Text <> "" Then TextBox13.Text += Environment.NewLine
        TextBox13.AppendText(_newNote)
    End Sub
    ...
    Class class_ClientHandler
        ...
        'Remove Client
        Public Function RemoveClient(ByVal _clientIP As String, Optional ByVal _doNotRaiseClientRemovedEvent As Boolean = False)
            Return FilteredRemoveClient(IPAddress.Parse(_clientIP), _doNotRaiseClientRemovedEvent)
        End Function
        Public Function RemoveClient(ByVal _clientIP As IPAddress, Optional ByVal _doNotRaiseClientRemovedEvent As Boolean = False)
            Return FilteredRemoveClient(_clientIP, _doNotRaiseClientRemovedEvent)
        End Function
    ...
    Private Sub EventHandler_ClientAdded(ByVal _clientIP As IPAddress) Handles ClientHandler.ClientAdded
        ...
        If Label21.Font.Bold Then LoadClientList()
    ...
    Class class_PlayersHandler
        ...
        Private vOwnerForm As Form4
        Sub New(ByRef _ownerForm As Form4)
            vOwnerForm = _ownerForm
        End Sub
        ...
        Class class_PlayerClass
            ...
            Public Property IPAddress As String
                ...
                End Set
            End Property
            Private pLastCheckInTime As Date
            Public Property LastCheckInTime As Date
                Get
                    Return pLastCheckInTime
                End Get
                Set(value As Date)
                    pLastCheckInTime = value
                End Set
            End Property
        ...
        'Client Check-In Ping ' Chi Test
        Public Function CheckInClient(ByVal _clientIP As String)
            Return FilteredCheckInClient(_clientIP)
        End Function
        Public Function CheckInClient(ByVal _clientIP As IPAddress)
            Return FilteredCheckInClient(_clientIP.ToString)
        End Function
        Private Function FilteredCheckInClient(ByVal _clientIP As String)
            For Each player In PlayersList
                If player.IPAddress.ToString = _clientIP Then
                    player.LastCheckInTime = Date.Now
                    Return ("Success")
                End If
            Next
            Return "Fail"
        ...
        'Remove Checked-Out Clients' Delta Test
        Public Sub RemoveAbsentPlayers()
            Dim now As Date = Date.Now
            Dim playersToRemove As New List(Of String)
            For Each player In PlayersList
                If player.LastCheckInTime = Nothing Then
                    CheckInClient(player.IPAddress)
                Else
                    Dim datediffvar = DateDiff(DateInterval.Second, player.LastCheckInTime, now)
                    If DateDiff(DateInterval.Second, player.LastCheckInTime, now) > 190 Then
                        playersToRemove.Add(player.IPAddress)
                    End If
                End If
            Next
            For Each player In playersToRemove
                vOwnerForm.ClientHandler.RemoveClient(player)
                vOwnerForm.UDPServerNotes("Client Timed Out")
            Next
            'i think i can do RemovePlayer(), and then have a timer check what clients aren't players anymore and then remove them
        End Sub
    End Class
    Private PlayerHandler As New class_PlayersHandler(Me)
    ...
    Private Sub TimeoutTimer_Tick(sender As System.Object, e As System.EventArgs) Handles TimeoutTimer.Tick
        PlayerHandler.RemoveAbsentPlayers()
    End Sub
#End Region
#Region "UDP Client"
    ...
    Private Sub Button3_Click(sender As System.Object, e As System.EventArgs) Handles Button3.Click
        ...
        clientPingTimer_timer.Interval = 30000
        'clientPingTimer_timer.Interval = 5000
        clientPingTimer_timer.Start()
        ConnectedPlayersList = New List(Of String)
        MyLocationFile = Program.Files.UserSettings.GetTaggedDataLine("Work Drive Path", 1) & "Net Streams\o\FAPI Data2.txt"
    ...
    Private Sub clientConnectionTimeout_sub(sender As System.Object, e As System.EventArgs) Handles clientConnectionTimeout_timer.Tick
        UDPClientNotes("Failed to connect to server")
    ...
    Private Sub DisconnectClientCode(Optional ByVal _sendLeaveMessage As Boolean = True)
        timer_OutputLocation.Stop()
        clientPingTimer_timer.Stop()
        If _sendLeaveMessage Then Client_UDP.SendUDP("[leave]") 'client usp was null when i clicked leave right after clicking join
    ...
    Private Sub UDPClientReceivedData_EventHandler(ByVal _receivedData As String, ByVal _senderIP As String, ByVal _portUsed As Integer) Handles Client_UDP.UDPServerDataReceived
        If _senderIP <> Client_UDP.ServerIP.ToString Then
            If tryRIPCheck Then
                ...
                End If
            End If
            UDPClientNotes("Nonserver message rejected from: " & _senderIP)
            'chat below
        Else
            If _receivedData = "[ssd]" Then 'server shutting down
                DisconnectClientCode(False)
                UDPClientNotes("Server Went Offline")
            ElseIf _receivedData = "[kicked]" Then
                DisconnectClientCode(False)
            ElseIf _receivedData = "[accepted]" Then
                clientConnectionTimeout_timer.Stop()
                UDPClientNotes("Connected")
            ...
            ElseIf _receivedData = "[ua]" Then
                DisconnectClientCode(False)
                UDPClientNotes("Your desired name is unavailable. Please try again with a different one.")
            ElseIf _receivedData = "[na]" Then
                UDPClientNotes("Your name has been accepted!")
            ...
            Else
                If tryRIPCheck Then
                    ...
                    End If
                End If
                UDPClientNotes("Nonchat message from server: " & _receivedData)
            End If
        End If
    End Sub
    ...
    Private WithEvents clientPingTimer_timer As New Timer
    Private Sub clientPingTimer_timer_Tick(sender As System.Object, e As System.EventArgs) Handles clientPingTimer_timer.Tick
        'UDPClientNotes("Pinging Server")
        Client_UDP.SendUDP("[bingo]") 'send ping
    End Sub
#End Region
        ...
        Try
            If My.Computer.FileSystem.FileExists(MyLocationFile) Then
                ...
                If Client_UDP IsNot Nothing Then
                    If Client_UDP.Status = "Hosting" Then
                        '  If RadioButton2.Checked Then
                        Client_UDP.SendUDP("[ml]" & MyX & ":" & MyY & ":" & MyZ & ":" & MyF)
                    End If
                ElseIf Server_UDP IsNot Nothing Then
                    If Server_UDP.Status = "Hosting" Then
...
#End Region
#Region "List Boxes with Context Menus"
    'Context Menu Code
    Private Sub ContextMenuStrip1_ItemClicked(sender As System.Object, e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles ContextMenuStrip1.ItemClicked
        If TabControl1.SelectedTab.Text = "Host Tab" Then
            If Label21.Font.Bold Then
            ...
            ElseIf Label25.Font.Bold Then
                ...
                End Select
            End If
        ElseIf TabControl1.SelectedTab.Text = "Guest Tab" Then
            If Label26.Font.Bold Then
                Select Case e.ClickedItem.Text
                    Case "Add Current Host"
                        Files.ClientSettings.AddToTaggedData("Host List") = TextBox2.Text
                        Files.ClientSettings.Save()
                        Files.ClientSettings.ReadFile()
                        LoadHostList()
                    Case "Remove Current Host"
                        Files.ClientSettings.RemoveDataFromTag("Host List") = TextBox2.Text
                        Files.ClientSettings.Save()
                        Files.ClientSettings.ReadFile()
                        LoadHostList()
                    Case "Remove Selected Host"
                        Files.ClientSettings.RemoveDataFromTag("Host List") = ListBox4.SelectedItem
                        Files.ClientSettings.Save()
                        Files.ClientSettings.ReadFile()
                        LoadHostList()
                    Case "Join Selected Host"
        ...
        End If
    End Sub
    Private Sub ContextMenuStrip1_Closed(sender As System.Object, e As System.Windows.Forms.ToolStripDropDownClosedEventArgs) Handles ContextMenuStrip1.Closed
        ContextMenuStrip1.Items.Clear()
    End Sub

    'Client Tab ' Host List, Buddy List, Ignore List
    Private Sub Label26_28_30_Click(sender As System.Object, e As System.EventArgs) Handles Label26.Click, Label28.Click, Label30.Click
        'unset unselected here
        If sender IsNot Label26 And Label26.Font.Bold Then
            Label26.Font = New Font(Label26.Font, FontStyle.Regular)
            ' ProgramLog = TextBox1.Text
        ElseIf sender IsNot Label28 And Label28.Font.Bold Then
            Label28.Font = New Font(Label28.Font, FontStyle.Regular)
        ElseIf sender IsNot Label30 And Label30.Font.Bold Then
            Label30.Font = New Font(Label30.Font, FontStyle.Regular)
        End If

        'set selected here
        If sender Is Label26 Then
            Label26.Font = New Font(Label26.Font, FontStyle.Bold)
            LoadHostList()
            '  TextBox1.Text = ProgramLog
        ElseIf sender Is Label28 Then
            Label28.Font = New Font(Label28.Font, FontStyle.Bold)
            ListBox4.Items.Clear()
            '  TextBox1.Text = UpdateNotes
        ElseIf sender Is Label30 Then
            Label30.Font = New Font(Label30.Font, FontStyle.Bold)
            ListBox4.Items.Clear()
        End If
    End Sub ' Host List, Buddy List, Ignore List
    Private Sub ListBox4_MouseUp(sender As System.Object, e As System.Windows.Forms.MouseEventArgs) Handles ListBox4.MouseUp
        If e.Button <> Windows.Forms.MouseButtons.Right Then Exit Sub
        If sender.SelectedIndex >= 0 Then ' if something is selected
            If Client_UDP Is Nothing Then
                ContextMenuStrip1.Items.Add("Join Selected Host")
            End If
            If Label26.Font.Bold Then 'host list label
                If ListBox4.SelectedItem <> TextBox2.Text Then
                    ContextMenuStrip1.Items.Add("Remove Selected Host")
                End If
            ElseIf Label28.Font.Bold Then ' buddy list label
                'ContextMenuStrip1.Items.Add("Remove")
            ElseIf Label30.Font.Bold Then ' ignore list label
                'ContextMenuStrip1.Items.Add("Remove")
            End If
        End If
        'If sender.SelectedIndex < 0 Then 'if nothing is selected
        '
        'End If
        If Label26.Font.Bold Then 'host list label
            If Client_UDP IsNot Nothing Then
                If Label8.Text = "Connected" Then
                    If ListBox4.Items.Contains(TextBox2.Text) Then
                        ContextMenuStrip1.Items.Add("Remove Current Host")
                    Else
                        ContextMenuStrip1.Items.Add("Add Current Host")
                    End If
                End If
            End If
        End If
        ...
        End If
    End Sub ' client tab's list box
    Private Sub LoadHostList()
        'GroupBox6.Text = "Client List"
        If CurrentUserName <> "" Then
            ListBox4.Items.Clear()
            For Each item In Files.ClientSettings.GetTaggedData("Host List")
                ListBox4.Items.Add(item)
            Next
        End If
    End Sub

    'Host Tab ' Client List, Black List, White List
    Private Sub Label21_23_25_Click(sender As System.Object, e As System.EventArgs) Handles Label21.Click, Label23.Click, Label25.Click
        'unset unselected here
        If sender IsNot Label21 And Label21.Font.Bold Then
            Label21.Font = New Font(Label21.Font, FontStyle.Regular)
            ' ProgramLog = TextBox1.Text
        ElseIf sender IsNot Label23 And Label23.Font.Bold Then
            Label23.Font = New Font(Label23.Font, FontStyle.Regular)
            ' UpdateNotes = TextBox1.Text
        ElseIf sender IsNot Label25 And Label25.Font.Bold Then
            Label25.Font = New Font(Label25.Font, FontStyle.Regular)
        End If

        'set selected here
        If sender Is Label21 Then
            Label21.Font = New Font(Label21.Font, FontStyle.Bold)
            LoadClientList()
        ElseIf sender Is Label23 Then
            Label23.Font = New Font(Label23.Font, FontStyle.Bold)
            LoadBlackList()
        ElseIf sender Is Label25 Then
            Label25.Font = New Font(Label25.Font, FontStyle.Bold)
            LoadWhiteList()
        End If
    End Sub ' Client List, Black List, White List
...
#Region "Program Logs Code"
    ...
    Dim UpdateNotes As String = _
        "Update Notes - Kairen 4.0.0.13 - Unreleased Version" & Environment.NewLine &
         Environment.NewLine & _
        "Update Notes - Kairen 4.0.0.12 - 11/6/2019" & Environment.NewLine & _
        "   -Changed UI : Moved, Added, Removed, and Adjusted UI Elements & Behaviors" & Environment.NewLine & _
        "   -Added Host List, Buddy List, Ignore List, and White List" & Environment.NewLine & _
        "   -Added Timeout Functionality to the Client and Server" & Environment.NewLine & _
        "   -Added AIS Option" & Environment.NewLine & _
        "   -Updated Program Update Timeout to November 5th, 2020" & Environment.NewLine & _
         Environment.NewLine & _
...
#Region "Get IP Code"
    ...
    Private Sub getiptimer_Tick(sender As System.Object, e As System.EventArgs) Handles getiptimer.Tick
        ...
        getiptimer.Stop()
    End Sub
    Dim WithEvents getipforRIPtimer As Timer
    Private Sub getipforRIPtimer_Tick(sender As System.Object, e As System.EventArgs) Handles getipforRIPtimer.Tick
        getipforRIPtimer.Stop()
        Dim posip As String = TryIPGet()
        Try
            IPAddress.Parse(posip)
            Log = "Detected Internet IP Address: " & posip
            Log = "Updating RIP..."
            'C:\Users\Kaynewihza\Desktop\UpdateMIP.ahk
            Dim path As String = "C:\Users\Kaynewihza\Desktop\UpdateRIP.ahk"
            Dim arg As String = posip
            Dim myProcess As New Process
            myProcess.StartInfo.WorkingDirectory = Microsoft.VisualBasic.Left(path, path.LastIndexOf("\"))
            myProcess.StartInfo.FileName = Microsoft.VisualBasic.Right(path, path.Length - path.LastIndexOf("\") - 1)
            myProcess.StartInfo.Arguments = arg
    ...
    Private Sub Label11_MouseUp(sender As System.Object, e As System.Windows.Forms.MouseEventArgs) Handles Label11.MouseUp
        ...
        End If
    End Sub
    Private Sub TryPutPublicIPinServerHostTextBox()
...
#End Region
    ...
    'RIP Stuff
    Private Sub UpdateRIPToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles UpdateRIPToolStripMenuItem.Click
        Log = "Attempting to update RIP.."
        Log = "Reading the webpage " & Chr(34) & "https://www.ip-adress.com/" & Chr(34) & " for your IP address.."
        getipforRIPtimer = New Timer
        getipforRIPtimer.Start()
    End Sub
    Private Sub CheckRIPToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles CheckRIPToolStripMenuItem.Click
        CheckForRIP()
    End Sub
    Dim WithEvents CheckRIPTimer As Timer
    Private Sub CheckForRIP()
        'if check for updates button checked rip, then i wouldn't need to recode this at all

        If (Year(Today) < 2020) Or (Year(Today) = 2020 And Month(Today) < 11) Or (Year(Today) = 2020 And Month(Today) = 11 And Today.Day <= 5) Then
            'do update
            CheckRIPTimer = New Timer
            CheckRIPTimer.Interval = 1
            CheckRIPTimer.Start()
        ElseIf (Year(Today) > 2020) Or (Year(Today) = 2020 And Month(Today) >= 11) Or (Year(Today) = 2020 And Month(Today) = 11 And Today.Day > 5) Then
            'do not update
            Log = "Cancelling RIP Check : For security, this won't RIP check past November, 2019 in case I do not control the domain after that"
        Else
            'error in my code! )';
            Log = "Cancelling RIP Check : An Error Occured While Trying Not to Y2K."
        End If
    End Sub
    Private Sub CheckRIPTimer_Tick(sender As System.Object, e As System.EventArgs) Handles CheckRIPTimer.Tick
        Dim testthis As Boolean = False
        If testthis = True Then Log = " ---> Test This is On <--- "

        CheckRIPTimer.Interval = 60000
        'this whole process needs multithreaded
        'Log = "Checking Version for Latest Kairen Release"
        Dim temp_rip As String = ReadLineFromFileOnline(WebsiteDomainAddress & "databits/kairenrip/rip.txt")
        If temp_rip = "error" Or temp_rip.Length > 15 Then
            'Log = "An error occured while checking RIP."
            If temp_rip.Length > 15 Then
                'Log = "Error: 16"
            ElseIf temp_rip = "error" Then
                'Log = "Error: 0"
            End If
            'DisplayControl.UpdateNotPresent()
            'Exit Sub
        ElseIf IPAddress.TryParse(temp_rip, IPAddress.Any) Then
        ...
        End If
        CheckRIPTimer.Stop()
        If testthis Then KairenLatestVersion = temp_rip
    End Sub
    Private Sub RIPOffToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles RIPOffToolStripMenuItem.Click
        Try
            Log = "Attempting to RIP Off.."
            'C:\Users\Kaynewihza\Desktop\UpdateMIP.ahk
            Dim path As String = "C:\Users\Kaynewihza\Desktop\UpdateRIP.ahk"
            Dim arg As String = "off"
            Dim myProcess As New Process
            myProcess.StartInfo.WorkingDirectory = Microsoft.VisualBasic.Left(path, path.LastIndexOf("\"))
            myProcess.StartInfo.FileName = Microsoft.VisualBasic.Right(path, path.Length - path.LastIndexOf("\") - 1)
            myProcess.StartInfo.Arguments = arg
            ...
            'If MyIPs.Contains(posip) = False Then MyIPs.Add(posip)
        Catch ex As Exception
            Log = "Error: Unable to RIP Off."
            'Log = "       You can google for it with this: " & Chr(34) & "What is my IP?" & Chr(34)
        End Try
    End Sub

    Private Sub Label32_Click(sender As System.Object, e As System.EventArgs) Handles Label32.Click
        If RHS.Substring(0, 2) = "11" Then
            If TabControl1.TabPages.Contains(TabPage2) And TextBox2.Enabled Then
                TabControl1.SelectedTab = TabPage2
                TextBox2.Text = RIP
                Button3.PerformClick()
            End If
        ...
        End If
    End Sub
    Private Function RIPValidCheck()
        Try
            IPAddress.Parse(RIP)
        Catch ex As Exception
            Return False
    ...
    'AIS Stuff
    Private Sub TurnOffAISToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles TurnOffAISToolStripMenuItem.Click
        SuppressReadyCheck = True
        If sender.Checked = True Then
            sender.Checked = False
            Files.UserSettings.SetTaggedDataLine("Turn Off AIS") = "false"
            Files.UserSettings.Save()
        ElseIf sender.Checked = False Then
            sender.Checked = True
            Files.UserSettings.SetTaggedDataLine("Turn Off AIS") = "true"
            Files.UserSettings.Save()
        End If
        SuppressReadyCheck = False
    End Sub
    ...
    Private Sub OffToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles OffToolStripMenuItem.Click
        TrySetRHS("00000000")
    End Sub
    Private Sub HostingToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles HostingToolStripMenuItem.Click
        TrySetRHS("10000000")
    End Sub
    Private Sub HostingJoinableToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles HostingJoinableToolStripMenuItem.Click
        TrySetRHS("11000000")
    End Sub
    Private Sub StreamingToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles StreamingToolStripMenuItem.Click
        TrySetRHS("00100000")
    End Sub
    Private Sub HostingStreamingToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles HostingStreamingToolStripMenuItem.Click
        TrySetRHS("10100000")
    End Sub
    Private Sub HostingJoinableStreamingToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles HostingJoinableStreamingToolStripMenuItem.Click
        TrySetRHS("11100000")
    End Sub
    Private Sub TrySetRHS(ByVal _value As String)
        Try
            Log = "Attempting to set RHS.. (" & _value & ")"
            Dim path As String = "C:\Users\Kaynewihza\Desktop\UpdateRHS.ahk"
            Dim arg As String = _value
            Dim myProcess As New Process
            myProcess.StartInfo.WorkingDirectory = Microsoft.VisualBasic.Left(path, path.LastIndexOf("\"))
            myProcess.StartInfo.FileName = Microsoft.VisualBasic.Right(path, path.Length - path.LastIndexOf("\") - 1)
            myProcess.StartInfo.Arguments = arg
        ...
        Catch ex As Exception
            Log = "Error: Unable to set RHS."
        End Try
    End Sub
    Dim WithEvents CheckRHSTimer As Timer
    Private Sub CheckRHS()
        'if check for updates button checked rip, then i wouldn't need to recode this at all

        If (Year(Today) < 2020) Or (Year(Today) = 2020 And Month(Today) < 11) Or (Year(Today) = 2020 And Month(Today) = 11 And Today.Day <= 5) Then
            'do update
            CheckRHSTimer = New Timer
            CheckRHSTimer.Interval = 1
            CheckRHSTimer.Start()
        ElseIf (Year(Today) > 2020) Or (Year(Today) = 2020 And Month(Today) >= 11) Or (Year(Today) = 2020 And Month(Today) = 11 And Today.Day > 5) Then
            'do not update
            Log = "Cancelling RHS Check : For security, this won't RHS check past November, 2019 in case I do not control the domain after that"
        Else
            'error in my code! )';
            Log = "Cancelling RHS Check : An Error Occured While Trying Not to Y2K."
        End If
    End Sub
    Private RHS As String = "00000000"
    Private Sub CheckRHSTimer_Tick(sender As System.Object, e As System.EventArgs) Handles CheckRHSTimer.Tick
        If RIP = "off" Then Exit Sub
        Dim testthis As Boolean = False
        If testthis = True Then Log = " ---> Test This is On <--- "

        CheckRHSTimer.Interval = 60000
        'this whole process needs multithreaded
        'Log = "Checking Version for Latest Kairen Release"
        Dim temp_rhs As String = ReadLineFromFileOnline(WebsiteDomainAddress & "databits/kairenrhs/rhs.txt")
        If temp_rhs = "error" Or temp_rhs = " " Or temp_rhs.Length > 15 Then
            'Log = "An error occured while checking RIP."
            If temp_rhs.Length > 15 Then
                'Log = "Error: 16"
            ElseIf temp_rhs = "error" Then
                'Log = "Error: 0"
            End If
            'DisplayControl.UpdateNotPresent()
            Exit Sub
        End If
        ' Randall is Hosting ' label
        If temp_rhs.Substring(0, 1) = "1" Then
            'If Microsoft.VisualBasic.Left(te'p_rhs, 1) = "1" Then
            'hosting game world
            Label32.Visible = True
        Else
        ...
        ' Randall is Streaming ' label
        If temp_rhs.Substring(2, 1) = "1" Then
            'If Microsoft.VisualBasic.Left(te'p_rhs, 1) = "1" Then
            'hosting game world
        ...
        RHS = temp_rhs
    End Sub
    Private Sub CheckRHSToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles CheckRHSToolStripMenuItem.Click
        CheckRHS()
    End Sub
    Private Sub UnsetRHSToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs)
        TrySetRHS("00000000")
        Exit Sub
        Try
            Log = "Attempting to unset RHS.. (0)"
            Dim path As String = "C:\Users\Kaynewihza\Desktop\UpdateRHS.ahk"
            Dim arg As String = "off"
            Dim myProcess As New Process
            myProcess.StartInfo.WorkingDirectory = Microsoft.VisualBasic.Left(path, path.LastIndexOf("\"))
        ...
        Catch ex As Exception
            Log = "Error: Unable to unset RHS."
        End Try
    End Sub
    Private Sub LocalRIPToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles LocalRIPToolStripMenuItem.Click
        For Each ip In System.Net.Dns.GetHostByName(System.Net.Dns.GetHostName).AddressList
            If MsgBox("Use " & ip.ToString & "?", MsgBoxStyle.YesNo, "Update Local RIP") = MsgBoxResult.Yes Then
                Try
                    IPAddress.Parse(ip.ToString)
                    Log = "Detected Local IP Address: " & ip.ToString
                    Log = "Updating RIP..."
                    'C:\Users\Kaynewihza\Desktop\UpdateMIP.ahk
                    Dim path As String = "C:\Users\Kaynewihza\Desktop\UpdateRIP.ahk"
                    Dim arg As String = ip.ToString
                    Dim myProcess As New Process
                    myProcess.StartInfo.WorkingDirectory = Microsoft.VisualBasic.Left(path, path.LastIndexOf("\"))
                    myProcess.StartInfo.FileName = Microsoft.VisualBasic.Right(path, path.Length - path.LastIndexOf("\") - 1)
                    myProcess.StartInfo.Arguments = arg
                    ...
                    'If MyIPs.Contains(posip) = False Then MyIPs.Add(posip)
                Catch ex As Exception
                    Log = "Error: Unable to do that at this time."
                    Log = "       You can fix this!" & Chr(34)
                End Try
                Exit Sub
            End If
        Next
    ...
    End Sub
    Private Sub InvitePlayerToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles InvitePlayerToolStripMenuItem.Click
        Dim playersip As String = InputBox("Input the person's IP Address", "Invite Player to Game", "192.168.1.5")
        Try
            IPAddress.Parse(playersip)
            Server_UDP.SendUDP("ri1", playersip)
        Catch ex As Exception

        End Try
    End Sub
    Private Sub NoNetPlayerToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles NoNetPlayerToolStripMenuItem.Click
        Dim playersip As String = InputBox("Input the person's IP Address", "nonet") ', "192.168.1.5")
        Try
            IPAddress.Parse(playersip)
            Server_UDP.SendUDP("nonet", playersip)
        Catch ex As Exception
    ...
    Private Sub Label33_Click(sender As System.Object, e As System.EventArgs) Handles Label33.Click
        Log = "Retrieving Randall's Streaming Link.."
        CheckStreamLinkTimer = New Timer
        CheckStreamLinkTimer.Interval = 100
        CheckStreamLinkTimer.Start()
    End Sub
    Dim WithEvents CheckStreamLinkTimer As Timer
    Private Sub CheckStreamLinkTimer_Tick(sender As System.Object, e As System.EventArgs) Handles CheckStreamLinkTimer.Tick
        CheckStreamLinkTimer.Stop()
        Dim link As String = ReadLineFromFileOnline(WebsiteDomainAddress & "databits/kairenstream/streamlink.txt")
        If link = "error" Then
            Log = "Error: Could not retrieve the stream's link at this time."
        Else
            Log = "Randall's Stream Link is: " & link
        End If
    End Sub
    Private Sub SetStreamLinkToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles SetStreamLinkToolStripMenuItem.Click
        Dim link As String = InputBox("What's your stream link?", "Stream Link Auto Updater")
        Try
            Log = "Attempting to update Stream Link to: " & link
            'C:\Users\Kaynewihza\Desktop\UpdateMIP.ahk
            Dim path As String = "C:\Users\Kaynewihza\Desktop\UpdateStreamLink.ahk"
            Dim arg As String = link
            Dim myProcess As New Process
            myProcess.StartInfo.WorkingDirectory = Microsoft.VisualBasic.Left(path, path.LastIndexOf("\"))
            myProcess.StartInfo.FileName = Microsoft.VisualBasic.Right(path, path.Length - path.LastIndexOf("\") - 1)
            myProcess.StartInfo.Arguments = arg
            ...
            'If MyIPs.Contains(posip) = False Then MyIPs.Add(posip)
        Catch ex As Exception
            Log = "Error: Unable to update Stream Link"
            'Log = "       You can google for it with this: " & Chr(34) & "What is my IP?" & Chr(34)
        End Try
    End Sub

    'Map Form Stuff
    Dim MapForm As New Form_Map
    Private Sub Button6_Click(sender As System.Object, e As System.EventArgs) Handles Button6.Click
        If MapTimer.Enabled = False Then
            MapForm.Show()
            MapTimer.Start()
            If MyLocationFile = "" Then MyLocationFile = Program.Files.UserSettings.GetTaggedDataLine("Work Drive Path", 1) & "Net Streams\o\FAPI Data2.txt"
        Else
            MapTimer.Stop()
            MapForm.Hide()
        End If
    End Sub
    Dim WithEvents MapTimer As New Timer
    Private Sub MapTimer_Tick(sender As System.Object, e As System.EventArgs) Handles MapTimer.Tick
        If timer_OutputLocation.Enabled Then
            MapForm.SetLocation(MyX, MyY)
        Else
            Try
                If My.Computer.FileSystem.FileExists(MyLocationFile) Then
                    MyLocationSR = New IO.StreamReader(MyLocationFile)
